/**
 * duckdb-bun — main-thread driver.
 *
 * Direct FFI bindings to DuckDB's C API using the modern chunk-based
 * API. Pure ESM, no build step on the JS side; the only artifact is a
...
// IMPORTANT — load order matters on Windows.
//
...
// called, NOT via our JS path discovery. By loading libduckdb first (and
// retaining its handle module-scoped so Bun can't finalize it), the
// shim's import binds to the already-loaded module by name. This is the
// "preload" pattern from the design doc — and on Unix it's a no-op
// (we'd be loading libduckdb in this order anyway).
//
// Both handles MUST be module-scoped values, not just `.symbols`
// destructured at the call site. If the dlopen wrapper objects become
// unreachable, Bun may call FreeLibrary on them at GC time, which on
// Windows would invalidate the shim's bound import.
const ddbLib = dlopen(libPath, {
  // Database lifecycle
  ...
  // Cancellation. Takes a connection handle (by value, BigInt) and
  // signals the running query to stop. Safe to call when no query is
  // running (no-op). Used by duckdb-bun/async to interrupt a worker-
  // blocked query from the main thread — same address space, shared
  // libduckdb state, so calling this from any thread on a connection
  // handle obtained from another thread is supported by libduckdb.
  // See v0.7 cancellation design in docs/rfcs/0001.
  duckdb_interrupt:  { args: ['u64'], returns: 'void' },
...
//      for contributors and source-clone users (Windows builds via build.ps1)
//   4. Untagged shim next to libduckdb.{so,dylib,dll} — historical install
...
// Retain the dlopen wrapper at module scope (see the comment on ddbLib).
const shimLib = shimPath ? dlopen(shimPath, {
  shim_fetch_chunk: { args: ['ptr'], returns: 'u64' },
}) : null;
const shim = shimLib ? shimLib.symbols : null;

// Fetch chunk: use shim (pointer-based) or fall back to direct call.
// The fallback works on macOS arm64 (hidden-pointer struct ABI happens
// to match Bun's 'ptr' marshaling). On Linux x64 / Windows / etc. the
// shim is required — without it, the FFI call would crash because the
// caller can't pass a 48-byte struct by value through Bun's marshaling.
const directFetchLib = !shim ? dlopen(libPath, {
...
// Raised when an async query is interrupted via AbortSignal. The
// `name` is set to 'AbortError' (Web standard convention used by
// fetch, ReadableStream, etc.) so existing abort-aware code paths
// recognize it via `err.name === 'AbortError'`. The class identity
// is still DuckDBError-derived so instanceof DuckDBError works.
//
// Only thrown by duckdb-bun/async. The sync subpath cannot deliver
...
class Connection {
  ...
  // Shortcut methods
  //
  // Mirror better-sqlite3 / bun:sqlite conventions but stay async because
...
// ==============================================================================
// Database extensions (shortcuts + transaction + lazy implicit Connection)
//
...
//
// NOT part of the public API. The leading underscore on the export
// is the convention; using these symbols outside the package is
// unsupported and may change without notice.
//
// Currently exposes:
//   - duckdb_interrupt(handle): used for AbortSignal-driven cancellation
//     in the async subpath. The worker stores its duckdb_connection
