...
//      tarball before publish, so `bun add bun-duckdb` users get a working
...
class Statement {
   ...
   * (matching what users expect from `await stmt.all(...)`) — bare `throw`
   * before returning the withLock promise would surface as a sync throw,
   * which is awkward for callers using `try/catch await`.
   */
  async all(params = []) {
