...
// connection gets a fresh generation token shipped to the main thread
// in the connect / txnBegin response. Main thread uses it to guard
// against a stale abort listener calling duckdb_interrupt on a freed
// connection pointer if connIds were ever reused (they aren't, ids
// are monotonic — but the token is defense-in-depth).
...
async function handleConnect(req: Req<'connect'>) {
  ...
  // Ship the raw duckdb_connection handle as an opaque BigInt. The
  // main thread caches it as an "interrupt capability" — when an
  // AbortSignal fires on an active op, the main thread calls
  // duckdb_interrupt(handle) directly via its own FFI binding while
  // this worker is blocked in FFI. Bun Workers share libduckdb state
  // in the same process, so this is a supported pattern. The handle
  // is never dereferenced from JS; we treat it as an opaque cookie.
  // Generation is incremented per conn so stale abort listeners can't
  // fire on a reused connId (currently ids are monotonic, so this is
  // belt-and-suspenders).
  return {
    connId: id,
...
async function handleTxnBegin(req: Req<'txnBegin'>) {
  ...
  // Include interrupt capability so AbortSignal can cancel sub-ops
  // running inside the transaction's connection (v0.7+).
  return {
    connId: id,
