...
// Generation counter for future AbortSignal cancellation: each new
// connection gets a fresh generation token so stale abort listeners
// on a closed-then-reopened connId can't accidentally interrupt the
// new connection.
...
async function handleConnect(req) {
  ...
  // capability" for the planned AbortSignal cancellation (will pass it
  // to duckdb_interrupt from the main thread while this worker is
  // blocked in FFI). Main never dereferences it from JS. Increment
  // generation per conn so stale abort listeners can't fire on a
  // reused connId.
  return {
    connId: id,
...
async function handleTxnBegin(req) {
  ...
  // Forward-compat: include interrupt capability for future cancellation
