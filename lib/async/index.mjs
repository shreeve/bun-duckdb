...
class AsyncDatabase {
  ...
  // Forward-compat for v0.5 cancellation (no functional use in v0.4.1).
  // The worker returns the raw duckdb_connection handle on each
  // `connect` / `txnBegin` response; we cache it here as an "interrupt
  // capability" so v0.5 can dlopen duckdb_interrupt on the main thread
  // and abort an in-flight query while the worker is blocked in FFI.
  // The spike that informed v0.5 architecture lives at
  // /tmp/cancel-spike (now gone), captured in
  ...
  async #ensureOpen() {
    ...
    this.#openPromise = (async () => {
      try {
        const { dbId } = await this._send({ op: OP.OPEN, path: this.#path });
        this.#dbId = dbId;
        this.#state = 'open';
      } catch (err) {
        this.#openFailed = err;
  ...
  async #ensureOpen() {
    ...
    this.#connectPromise = (async () => {
      try {
        ...
        // Forward-compat for v0.5: store the interrupt handle if the
        // worker sent one. Ignored in v0.4.1; v0.5 will use it to call
        // duckdb_interrupt from the main thread on AbortSignal.abort.
